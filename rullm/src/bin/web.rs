use std::{ops::Deref as _, path::PathBuf, sync::Arc};
...
async fn handle_socket(mut socket: WebSocket, env: Arc<Env>) {
    ...
    if let Some(Ok(Message::Pong(_))) = socket.recv().await {
        let mut chatter = HtmxChat { socket };
        ...
        // Start the main loop
        loop {
            match chatter.recv().await {
                Ok(Some(msg)) => {
                    if let Err(err) = process_message(env, &mut chatter, &mut messages, msg).await {
                        warn!("{:?}", err);
                        return;
                    }
                }
...
async fn process_message(
    env: &Env,
    chatter: &mut HtmxChat,
    messages: &mut Vec<ChatCompletionRequestMessage>,
    msg: HtmxMessage,
) -> anyhow::Result<()> {
    chatter
        .send(html! { div class="user-message bg-blue-500 text-white self-end rounded-xl rounded-br-none p-3 max-w-3/4 break-words" { (msg.chat_message) } })
    .await?;

    messages.push(
        ChatCompletionRequestUserMessageArgs::default()
            .content(msg.chat_message.clone())
            .build()?
            .into(),
    );
    ...
    loop {
        ...
        if tool_calls.is_empty() {
            ...
            chatter
                .send(html! { div #assistant class="assistant-message bg-gray-200 text-black self-start rounded-xl rounded-bl-none p-3 max-w-3/4 break-words" { (assistant_response)} })
            .await?;
